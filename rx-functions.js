...
function reduceInsteadOfFilter() {
	...
	// 				
	// 				if (accWidth) boxart = accWidth < nextWidth ? acc : next;
	// 				
	// 				return boxart;
	// 			}).map(({ url: boxart }) => {
	// 				return { id, title, boxart };
...
representative of the title as a whole. Notice that both the boxarts and interestingMoments arrays 
are located at the same depth in the tree. Retrieve the time of the middle interesting moment and 
the smallest box art url simultaneously with zip(). Return an {id, title, time, url} object for 
each video.
*/
